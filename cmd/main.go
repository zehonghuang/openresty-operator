...
func main() {
	...
	flag.BoolVar(&enableHTTP2, "enable-http2", false,
		"If set, HTTP/2 will be enabled for the metrics and webhookserver servers")
	opts := zap.Options{
		Development: true,
	}
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:       "8248f459.chillyroom.com",
	...
	if err = (&controller.OpenRestyReconciler{
		...
		Recorder: mgr.GetEventRecorderFor("openresty-controller"),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "OpenResty")
		os.Exit(1)
	}
	if err = (&controller.UpstreamReconciler{
		Client:   mgr.GetClient(),
		Scheme:   mgr.GetScheme(),
		Recorder: mgr.GetEventRecorderFor("upstream-controller"),
	}).SetupWithManager(mgr); err != nil {
	...
	if err = (&controller.LocationReconciler{
		Client:   mgr.GetClient(),
		Scheme:   mgr.GetScheme(),
		Recorder: mgr.GetEventRecorderFor("location-controller"),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "Location")
		os.Exit(1)
	}
	if err = (&controller.ServerBlockReconciler{
		Client:   mgr.GetClient(),
		Scheme:   mgr.GetScheme(),
		Recorder: mgr.GetEventRecorderFor("serverblock-controller"),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "ServerBlock")
		os.Exit(1)
	}
	...
	if err = (&controller.NormalizeRuleReconciler{
		Client:   mgr.GetClient(),
		Scheme:   mgr.GetScheme(),
		Recorder: mgr.GetEventRecorderFor("normalize-rule-controller"),
	}).SetupWithManager(mgr); err != nil {
